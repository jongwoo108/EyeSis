"""
이미지 배치 인식 스크립트 (All Banks - Base + Masked + Dynamic)
...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "test_results_masked_easy"  # Base + Masked Bank
...
def process_single_image(image_path: Path, db) -> Dict:
    ...
    annotation = {
        ...
        "banks_used": ["base", "masked", "dynamic"],  # 사용된 Bank 명시
        "faces": []
    }
    ...
    for detection in detections:
        ...
        bank_type = detection.get("bank_type", "unknown")
        
        # Bank 타입 표시 추가
        bank_emoji = {"base": "📌", "masked": "😷", "dynamic": "🔄"}.get(bank_type, "❓")
        label = f"{name} ({confidence}%) {bank_emoji}"
        ...
        face_annotation = {
            ...
            "bank_type": bank_type  # 어떤 Bank에서 매칭되었는지
        }
...
def main():
    """메인 함수"""
    print("=" * 70)
    print("🖼️  이미지 배치 인식 스크립트 (All Banks)")
    print("=" * 70)
    ...
    # 5. 데이터베이스 연결 및 데이터 로드 (All Banks 사용)
    print("🗄️  데이터베이스 연결 중...")
    print("   ✅ Base + Masked + Dynamic Bank 모두 사용")
    db = SessionLocal()
    try:
        load_persons_from_db(db)
        print("   ✅ 데이터베이스 로드 완료 (All Banks)")
        
        # Bank 상태 출력
        from backend.main import gallery_base_cache, gallery_masked_cache, gallery_dynamic_cache
        print(f"\n📊 Bank 상태:")
        for person_id in gallery_base_cache.keys():
            base_count = gallery_base_cache.get(person_id, np.array([])).shape[0] if gallery_base_cache.get(person_id) is not None and gallery_base_cache.get(person_id).ndim == 2 else 1
            masked_count = gallery_masked_cache.get(person_id, np.array([])).shape[0] if gallery_masked_cache.get(person_id) is not None and gallery_masked_cache.get(person_id).ndim == 2 else 0
            dynamic_count = gallery_dynamic_cache.get(person_id, np.array([])).shape[0] if gallery_dynamic_cache.get(person_id) is not None and gallery_dynamic_cache.get(person_id).ndim == 2 else 0
            print(f"   - {person_id}: base={base_count}, masked={masked_count}, dynamic={dynamic_count}")
        
    except Exception as e:
        print(f"   ⚠️ 데이터베이스 로드 실패: {e}")
        print("   파일 시스템에서 로드 시도...")
        from backend.main import load_persons_from_embeddings
        load_persons_from_embeddings()
        print("   ✅ 파일 시스템 로드 완료 (All Banks)")
    ...
    total_faces = 0
    processed_count = 0
    bank_usage_stats = {"base": 0, "masked": 0, "dynamic": 0, "unknown": 0}
    ...
    for image_path in image_files:
        try:
            ...
            # Bank 사용 통계
            for face in result["annotation"]["faces"]:
                bank_type = face.get("bank_type", "unknown")
                bank_usage_stats[bank_type] = bank_usage_stats.get(bank_type, 0) + 1
    ...
    print(f"\n📊 Bank 사용 통계:")
    print(f"   - Base Bank: {bank_usage_stats.get('base', 0)}개")
    print(f"   - Masked Bank: {bank_usage_stats.get('masked', 0)}개")
    print(f"   - Dynamic Bank: {bank_usage_stats.get('dynamic', 0)}개")
    print(f"   - Unknown: {bank_usage_stats.get('unknown', 0)}개")
    print(f"\n📁 결과 저장 위치:")
